        ...
        public async Task<IActionResult> Import()
        {
            ...
            foreach (Category c in db.Categories.Include(c => c.Products))
            {
                ...
                if (cp == null)
                {
                    ...
                    Guid categoriesFolderID = (await api.Media.GetAllFoldersAsync()).First(folder => folder.Name == "Categories").Id;
                    // find image with correct filename for category id
                    var image = (await api.Media.GetAllByFolderIdAsync(categoriesFolderID)).First(media => media.Type == MediaType.Image && media.Filename == $"category{c.CategoryID}.jpeg");
                    ...
                    if (cp.Products.Count == 0)
                    {
                        ...
                        cp.Products = c.Products.Select(p => new ProductRegion 
                            {
